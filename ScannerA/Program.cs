...
namespace ScannerA
{
    class Program
    {
        ...
        {
            Console.Title = "Scanner A"; // Set console window title
            ...
            // Normalize path to handle spaces and invalid characters
            ...
            try
            {
                ...
                await Task.Factory.StartNew(() => SendDataToMaster("agent1"));
            }
            catch (Exception ex)
            {
                LogError($"Error in Scanner A: {ex.Message}");
            }

            LogHeader("Scanner A Completed");
